...
<body class="bg-ghost-white">
  <div class="p-4 sm:ml-64 h-screen flex flex-col">
    <div class="pt-4 px-5 mt-14 flex-grow flex flex-col">
        ...
        <ul class="flex flex-wrap -mb-px text-sm font-medium text-center transition duration-300 ease-in-out"
          ...
          <li class="me-2" role="presentation">
            <button
              ...
              id="users-table-tab" data-tabs-target="#users" type="button" role="tab" aria-controls="users"
              aria-selected="false">Users</button>
          </li>
          <li class="me-2" role="presentation">
            <button
              class="inline-block px-4 py-2 hover:bg-ghost-lavender transition duration-300 ease-in-out border-b-2 rounded-t-lg"
              id="programs-table-tab" data-tabs-target="#programs" type="button" role="tab" aria-controls="programs"
              aria-selected="false">Programs</button>
          </li>
          <li class="me-2" role="presentation">
            <button
              class="inline-block px-4 py-2 hover:bg-ghost-lavender transition duration-300 ease-in-out border-b-2 rounded-t-lg"
              id="courses-table-tab" data-tabs-target="#courses" type="button" role="tab" aria-controls="courses"
      ...
      <div id="tab-content">
        <!-- Users Table -->
        <div class="hidden relative overflow-x-auto sm:rounded-lg" id="users" role="tabpanel"
          ...
          <div class="max-h-[480px] overflow-y-auto">
            <table class="w-full text-sm text-left rtl:text-right text-deep-koamaru">
              ...
              <tbody>
                ...
                $sql = "SELECT *, CASE WHEN UserType = 'Student' THEN StudentNum ELSE EmployeeNum END AS UserID FROM UserArchives_Tbl ORDER BY archived_at DESC";
                ...
                if ($result->num_rows > 0) {
                  ...
                  while ($row = $result->fetch_assoc()) {
                    ...
                    echo '<th scope="row" class="px-6 py-3 font-medium text-deep-koamaru whitespace-nowrap">' . $row["UserID"] . "</th>";
                    echo '<td class="px-6 py-3">' . $row["FirstName"] . " " . $row["MiddleName"] . " " . $row["LastName"] . " " . $row["Suffix"] . "</td>";
                    echo '<td class="px-6 py-3">' . $row["UserType"] . "</td>";
                    echo '<td class="px-6 py-3">' . $row["Username"] . "</td>";
                    echo '<td class="px-6 py-3">' . $row["Email"] . "</td>";
                    echo '<td class="px-6 py-3">' . $row["archived_at"] . "</td>";
                    echo "</tr>";
                  }
                } else {
        ...
        <!-- // Users Table -->
        <!-- Programs Table -->
        <div class="hidden relative overflow-x-auto sm:rounded-lg" id="programs" role="tabpanel"
          aria-labelledby="programs-table-tab">
          <div class="max-h-[480px] overflow-y-auto">
            <table class="w-full text-sm text-left rtl:text-right text-deep-koamaru">
              <thead class="text-xs text-corn-flower-blue uppercase bg-ghost-lavender sticky top-0">
                <tr>
              ...
              <tbody>
                <?php
                include '../models/config.php'; //connection ng db
                $sql = "SELECT * FROM ProgramArchives_Tbl ORDER BY archived_at DESC";
                $result = $conn->query($sql);
                ...
                if ($result->num_rows > 0) {
                  ...
                  while ($row = $result->fetch_assoc()) {
                    echo "<tr class='bg-white border-b border-indigo-100'>";
                    echo '<th scope="row" class="px-6 py-3 font-medium text-deep-koamaru whitespace-nowrap">' . $row["ProgramID"] . "</th>";
                    echo '<td class="px-6 py-3">' . $row["ProgramCode"] . "</td>";
                    echo '<td class="px-6 py-3">' . $row["ProgramTitle"] . "</td>";
                    echo '<td class="px-6 py-3">' . $row["archived_at"] . "</td>";
                    echo "</tr>";
                  }
                } else {
        ...
        <!-- // Programs Table -->
        <!-- Courses Table -->
        <div class="hidden relative overflow-x-auto sm:rounded-lg" id="courses" role="tabpanel"
          aria-labelledby="courses-table-tab">
          <div class="max-h-[480px] overflow-y-auto">
            <table class="w-full text-sm text-left rtl:text-right text-deep-koamaru">
              <thead class="text-xs text-corn-flower-blue uppercase bg-ghost-lavender sticky top-0">
                <tr>
